...
class ASdbCategoryUtil:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Check if the category file already exist, if not, download them
        if not saved_category_file.exists():
            try:
                self.io_helper.logger.info(
                    f"Downloading ASdb category dataset from {category_url} to {saved_category_file}"
                )
                r = requests.get(category_url, timeout=5)
            except requests.exceptions.Timeout:
                self.io_helper.logger.error(
                    f"Could not download ASdb category dataset from {saved_category_file}, cannot proceed"
                )
                raise
            else:
                saved_category_file.write_text(r.text)
        
        # Check if the main dataset file exists, if not, download it
        if not saved_data_file.exists():
            try:
                self.io_helper.logger.info(
                    f"Downloading ASdb dataset from {data_url} to {saved_data_file}"
                )
                r = requests.get(data_url, timeout=5)
            except requests.exceptions.Timeout:
                self.io_helper.logger.error(
                    f"Could not download ASdb dataset from {data_url}, cannot proceed"
                )
                raise
            else:
        ...
        df_l1["cat_idx"] = df_l1["cat_layer"].str.extract(r"Category (\d+)")[0].astype(int)
        df_l2["cat_idx"] = df_l2["cat_layer"].str.extract(r"Category (\d+)")[0].astype(int)
    ...
    def _build_category_dict(self, category_url: str | Path) -> dict[str, list[str]]:
        ...
        Returns:
            dict[str, list[str]]:
                ...
                Example: {"Computer and Information Technology": ["Internet Service Provider (ISP)", "Phone Provider", ...], 
        ...
        try:
            ...
            self.io_helper.logger.error(f"Category file not found: {category_url}")
            raise                            
        except KeyError:
            self.io_helper.logger.error(f"Invalid category format in file: {category_url}")
            raise
    ...
    def find_ases_in_category(
        ...
        Args:
            layer1_category: Name of the Layer-1 category to match.
            layer2_category: Name of the Layer-2 category to match (if any).
